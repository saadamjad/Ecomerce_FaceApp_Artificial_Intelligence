...
class AddSlider extends Component {
    ...
    if(this.state.avatarURL==undefined)
  {
   ...
   // console.log(productList.push(Math.floor(Math.random()*10000000)))
        var  SliderID= Math.floor(Math.random()*10000000)
         firebase.firestore().collection("Slider").doc(SliderID.toString()).set({
           image:imageList
...
//    this.componentDidMount()
   this.setState({downloadURLs:[],good:true,showImageError:false, visible:true,msgType:'success',massege:'Slider Added Successfully',open:false})
   setSubmitting(false);
  }
